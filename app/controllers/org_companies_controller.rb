class OrgCompaniesController < ApplicationController
  ...
  def create
    ...
    if !OrgCompany.exists?(name: company_register_params["name"], typ_company_id: company_register_params["typ_companies"]["id"])
      ...
      #Create a contact in the db
      @contact = OrgContact.create(org_company_id: @company.id) 
  ...
  def edit
    #Find the company we're dealing with
    ...
    # Build the contact input fields associated with company
    @company.org_contacts.build(@contactInfo) 
  end

  def show
    #Find the company we are dealing with
    @company = OrgCompany.find(params[:id]) 
    # Find the type of the company
  ...
  def update
    ...
    #Find/Create a contact in the db if it doesn't exist
    @contact = OrgContact.find_or_create_by(org_company_id: @company.id, org_person_id: nil) 
    
    # If the update was not sucessful, we need the previous record data to rerender the edit page
  ...
  def company_edit_params
    params.require(:org_company).permit(:name, :avatar, :description, org_contacts_attributes: [:address1, :address2, :city, {typ_countries: :id}, {typ_regions: :id}, :postal_code, :email, :usiness_number, :cell_number])
  end
